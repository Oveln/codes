...
struct SegmentTree {
    ...
    void _build(int now, int L, int R, vector<LL>* b) {
        ...
        a[now] = {a[now * 2].w + a[now * 2 + 1].w, 0, 0};
    }
    ...
    void down_lazy(int x, int L, int R) {
        if (a[x].change_lazy != 0) {
            ...
            a[Lson] = {a[x].change_lazy * (mid - L + 1), a[x].change_lazy, 0};
            a[Rson] = {a[x].change_lazy * (R - mid), a[x].change_lazy, 0};
            a[x].change_lazy = 0;
        }
        if (a[x].add_lazy != 0) {
            int mid = (L + R) / 2, Lson = x * 2, Rson = x * 2 + 1;
    ...
    void _modify(int now, int L, int R, int ML, int MR, int x) {
        ...
        int mid = (R + L) / 2;
        if (MR <= mid)
            _modify(now * 2, L, mid, ML, MR, x);
        else if (ML > mid)
            _modify(now * 2 + 1, mid + 1, R, ML, MR, x);
        else
            _modify(now * 2, L, mid, ML, mid, x),
                _modify(now * 2 + 1, mid + 1, R, mid + 1, MR, x);
    ...
    void _add(int now, int L, int R, int AL, int AR, int x) {
        ...
        int mid = (R + L) / 2;
        if (AR <= mid)
            _add(now * 2, L, mid, AL, AR, x);
        else if (AL > mid)
            _add(now * 2 + 1, mid + 1, R, AL, AR, x);
        else
            _add(now * 2, L, mid, AL, mid, x),
                _add(now * 2 + 1, mid + 1, R, mid + 1, AR, x);
        a[now].w = a[now * 2].w + a[now * 2 + 1].w;
    }
    LL query(int L, int R) { return _query(1, 1, size, L, R); }
    void modify(int L, int R, LL x) { _modify(1, 1, size, L, R, x); }
    void add(int L, int R, LL x) { _add(1, 1, size, L, R, x); }
};
struct Result {
