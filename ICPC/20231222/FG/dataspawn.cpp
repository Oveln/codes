...
char generate_random_operator() { return operators[rand() % 4]; }

int generate_random_number() { return rand() % 900 + 100; }
...
void generate_expression(int length, bool canp) {
    ...
    printf("%c", c);
    if (isp)
        printf("%d", (rand() % 1000) * (rand() % 1000) * (rand() % 1000));
    else
        generate_expression(right_length, isp);
    //    if (isp) printf("%d", (rand()%10)); else
    //    generate_expression(right_length,isp);
    if (flg) printf(")");
}
