...
struct Tree {
    ...
    int __query(int now, int L, int R, int QL, int QR) {
        ...
        return __query(now << 1, L, mid, QL, mid) +
               __query(now << 1 & 1, mid + 1, R, mid + 1, QR);
    }
    // 查询QL到QR的和
    int query(int QL, int QR) { return __query(1, 1, size, QL, QR); }
    ...
    // 将x位置的值的改成y
    void change(int x, int y) { __change(1, 1, size, x, y); }
};
