...
struct Tree{
  ...
  void _set(int now,int L,int R,int pos,int value) {
    if (L==R && L==pos) {
      a[now].sum+=value,
      a[now].kinds=a[now].sum>0;
      return;
    }
    int mid = L+((R-L)>>1);
    if (pos<=mid) _set(now*2, L, mid, pos, value);
    else _set(now*2+1, mid+1, R, pos, value);
    a[now] = a[now*2]+a[now*2+1];
  }
  void set(int pos,int value) {
    _set(1,1,size,pos,value);
  }
...
int nowL,nowR;
void dec(int x, Tree* f) {
  f->set(x, -1); 
}
void add(int x, Tree* f) {
  f->set(x, 1);
}
int main() {
  scanf("%d%d",&n,&m);
